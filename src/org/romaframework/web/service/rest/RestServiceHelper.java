...
public class RestServiceHelper {
	...
	public static void invokeRestService(HttpServletRequest request, HttpServletResponse response) {
		if (request.getSession().getAttribute(RestServiceConstants.SESSION_I18N_ERROR_ATTRIBUTE_NAME) != null) {
			String errorMessage = Roma.i18n().get((String) request.getSession().getAttribute(RestServiceConstants.SESSION_I18N_ERROR_ATTRIBUTE_NAME));
	...
	protected static void invokeService(HttpServletRequest request, HttpServletResponse response, String serviceName, String serviceOperation, String[] parameters) {
		...
		try {
			...
			sendToServiceErrorPage(Roma.i18n().get("RestServiceHelper.baseErrorMessage.error"), ie, request, response);
		} catch (IllegalAccessException iae) {
			log.error("Unable to access the service " + serviceName, iae);
			sendToServiceErrorPage(Roma.i18n().get("RestServiceHelper.baseErrorMessage.error"), iae, request, response);
			...
			sendToServiceErrorPage(Roma.i18n().get("RestServiceHelper.baseErrorMessage.error"), ite, request, response);
		} catch (UnsupportedOperationException uoe) {
			log.error("Unable to found the operation " + serviceOperation + " in service " + serviceName, uoe);
			sendToServiceErrorPage(Roma.i18n().get("RestServiceHelper.operationNotFound.error"), uoe, request, response);
		} catch (NoSuchMethodException e) {
			log.error("Unable to found the operation " + serviceOperation + " in service " + serviceName, e);
			sendToServiceErrorPage(Roma.i18n().get("RestServiceHelper.operationNotFound.error"), e, request, response);
		}
	}
